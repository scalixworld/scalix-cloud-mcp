...
2. **Verify the endpoint.** Call the open MCP discovery (no auth needed): `curl -s -X POST https://api.scalix.world/v1/mcp -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","id":1,"method":"tools/list"}'` and confirm a non-empty tool catalog comes back — 50 tools at the time of writing. Check that the list is there, not that it matches a number: the catalog changes, and the count in this file will lag it. This proves connectivity independent of the key. (The plain-HTTP `GET /api/v1/mcp` listing requires auth — don't use it for this step.)
...
If every step passes, summarise what is now available: 50 platform MCP tools, the REST API, and the CLI if installed.
